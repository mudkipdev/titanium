...
class NowPlaying(commands.Cog):
    ...
    @app_commands.command(
        name="now-playing", description="Show current activity / now playing status."
    )
    ...
    async def now_playing(
        ...
        if user.mutual_guilds is None or len(user.mutual_guilds) == 0:
            ...
            embed = discord.Embed(
                title="No Mutual Guilds",
                description=f"Titanium must be in a mutual guild with {user.mention if user_set else 'you'} to be able to see their status.",
                color=Color.red(),
            )
        ...
        if member.activity is None:
            ...
            if (
                isinstance(activity, discord.Spotify)
                and self.bot.tokens["spotify-api-id"] != ""
                and self.bot.tokens["spotify-api-secret"] != ""
            ):  # Spotify
                ...
                if activity.details:
                    ...
                    embed.set_footer(
                        text=f"@{user.name} - {activity.name}",
                    ...
                    try:
                        view.add_item(
                            discord.ui.Button(
                                url=f"https://www.google.com/search?q={quote_plus(activity.details)}+{quote_plus(activity.state)}+{quote_plus(activity.small_image_text)}",
