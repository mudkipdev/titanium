...
class Videos(commands.Cog):
    ...
    async def video_to_gif(
        ...
        if file.content_type.split("/")[0] == "video":  # Check if file is a video
                ...
                try:
                    ...
                    if proc.returncode == 0:
                        ...
                        embed = discord.Embed(
                            ...
                            description=f"Video converted to GIF.",
                            color=Color.green(),
                        )
                        embed.set_footer(
                            text=f"@{interaction.user.name}",
            ...
            embed = discord.Embed(
                title="Error",
                description=f"I think you attached an **image.** To convert an image to GIF, use the `/image to-gif` command, or right click on a message, select apps, then click Convert to GIF.",
                color=Color.red(),
            )
            embed.set_footer(
                text=f"@{interaction.user.name}",
            ...
            embed = discord.Embed(
                title="Error",
                description=f"Your file is not a video.",
                color=Color.red(),
            )
            embed.set_footer(
                text=f"@{interaction.user.name}",
