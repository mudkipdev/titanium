...
class Images(commands.Cog):
        ...
        if (
            ...
            if file.size < 20000000:  # 20MB file limit
                if (
                    ...
                    embed = discord.Embed(
                        title="Error",
                        description=f"Please provide a scale, target width or target height.",
                        color=Color.red(),
                    )
                    embed.set_footer(
                        text=f"@{interaction.user.name}",
                ...
                embed = discord.Embed(
                    title="Error",
                    description=f"Your file is too large. Please ensure it is smaller than 20MB.",
                    color=Color.red(),
                )
                embed.set_footer(
                    text=f"@{interaction.user.name}",
            ...
            embed = discord.Embed(
                title="Error",
                description=f"Your file is not a static image.",
                color=Color.red(),
            )
            embed.set_footer(
                text=f"@{interaction.user.name}",
    ...
    async def gif_image(
        ...
        if (
            ...
            if file.size < 20000000:  # 20MB file limit
                ...
                with Image.open(image_data) as im:
                    ...
                    embed = discord.Embed(
                        ...
                        description=f"Image converted to GIF.",
                        color=Color.green(),
                    )
                    embed.set_footer(
                        text=f"@{interaction.user.name}",
            ...
            embed = discord.Embed(
                title="Error",
                description=f"I think you attached a **video.** To convert a video to GIF, use the `/video to-gif` command.",
                color=Color.red(),
            )
            embed.set_footer(
                text=f"@{interaction.user.name}",
                        ...
                        if file.size < 20000000:  # 20MB file limit
                            ...
                            with Image.open(image_data) as im:
                                ...
                                converted_file = discord.File(
                                    fp=gif_data, filename=f"image.gif"
                                )
    ...
    async def deepfry_image(
        ...
        if (
            ...
            if file.size < 20000000:  # 20MB file limit
                ...
                # Send resized image
                embed = discord.Embed(
                    title="Done", description=f"Image deepfried.", color=Color.green()
                )
                embed.set_footer(
                    text=f"@{interaction.user.name}",
                    icon_url=interaction.user.display_avatar.url,
