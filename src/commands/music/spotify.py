...
class Spotify(commands.Cog):
    ...
    spotifyGroup = app_commands.Group(
        name="spotify",
    ...
    @app_commands.describe(
        search_type="The type of media you are searching for. Supported types are song, artist and album."
    )
        ...
        if search_type.value == "song":
                ...
                for item in result["tracks"]["items"]:
                    ...
                    options_list.append(
                        discord.SelectOption(
                            label=label, description=description, value=i
                        )
                    )
                    i += 1
            ...
            if len(result["albums"]["items"]) == 0:
                ...
                for item in result["albums"]["items"]:
                    ...
                    for artist in item["artists"]:
                        if artist_string == "":
                            artist_string = discord.utils.escape_markdown(artist["name"])
                        else:
                            artist_string += f", {discord.utils.escape_markdown(artist['name'])}"
                    ...
                    options_list.append(
                        discord.SelectOption(
                            label=title, description=description, value=i
                        )
                    )
                    i += 1
        ...
        try:
            if "track" in url:
                ...
                if result["album"]["images"] != None:
                    ...
                    if (
                        result["album"]["images"][0]["height"] == None
                        or result["album"]["images"][0]["width"] == None
                    ):
                        embed = discord.Embed(
                ...
                if result["images"] != None:
                    if (
                        result["images"][0]["height"] == None
                        or result["images"][0]["width"] == None
                    ):
                        embed = discord.Embed(
                            title=f"{result['name']} ({artist_string}) - Album Art",
                            description="Viewing highest quality (Resolution unknown)",
