...
class Fireboard(commands.Cog):
    ...
    async def setup(self):
        async with self.fireboard_pool.acquire() as sql:
            if (
                await sql.fetchone(
                    f"SELECT name FROM sqlite_master WHERE type='table' AND name='fireMessages';"
                )
                is None
            ):
            ...
            if (
                await sql.fetchone(
                    f"SELECT name FROM sqlite_master WHERE type='table' AND name='fireSettings';"
                )
                is None
            ):
            ...
            if (
                await sql.fetchone(
                    f"SELECT name FROM sqlite_master WHERE type='table' AND name='fireChannelBlacklist';"
                )
                is None
            ):
            ...
            if (
                await sql.fetchone(
                    f"SELECT name FROM sqlite_master WHERE type='table' AND name='fireRoleBlacklist';"
                )
                is None
            ):
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        try:
            ...
            if payload.message_id in [message[1] for message in self.fire_messages]:
                ...
                async with self.fireboard_pool.acquire() as sql:
                    ...
                    if react_count is not None:
                        await sql.execute(
                            f"UPDATE fireMessages SET reactionAmount = ? WHERE msgID = ?",
                            (
                                react_count,
                                payload.message_id,
                            ),
                        ...
                        await sql.execute(
                            f"UPDATE fireMessages SET reactionAmount = reactionAmount + 1 WHERE msgID = ?",
                            (payload.message_id,),
                        )
                        await self.refresh_fire_lists()
            ...
            async with self.fireboard_pool.acquire() as sql:
                ...
                await sql.execute(
                    f"INSERT INTO fireMessages (serverID, msgID, boardMsgID, reactionAmount) VALUES (?, ?, ?, ?)",
                    (
                        payload.guild_id,
                        payload.message_id,
                        board_message.id,
        ...
        try:
            ...
            if payload.message_id in [message[1] for message in self.fire_messages]:
                ...
                if message[3] < react_minimum:
                    ...
                    async with self.fireboard_pool.acquire() as sql:
                        await sql.execute(
                            f"DELETE FROM fireMessages WHERE msgID = ?",
                            (payload.message_id,),
                        )
                        await sql.commit()
                        ...
                        async with self.fireboard_pool.acquire() as sql:
                            try:
                                ...
                                try:
                                    ...
                                    await sql.execute(
                                        f"DELETE FROM fireSettings WHERE serverID = ?",
                                        (payload.guild_id,),
                                    )
                                    await sql.commit()
                                    self.locked_messages.remove(payload.message_id)
                ...
                if server[0] == channel.guild.id:
                    if server[3] == channel.id:
                        async with self.fireboard_pool.acquire() as sql:
                            ...
                            await sql.execute(
                                f"DELETE FROM fireMessages WHERE serverID = ?",
                                (channel.guild.id,),
                            )
                            await sql.execute(
                                f"DELETE FROM fireSettings WHERE serverID = ?",
            ...
            for server in self.fire_settings:
                if server[0] == guild.id:
                    async with self.fireboard_pool.acquire() as sql:
                        # Delete fireboard config
                        await sql.execute(
                            f"DELETE FROM fireMessages WHERE serverID = ?", (guild.id,)
                        )
                        await sql.execute(
                            f"DELETE FROM fireSettings WHERE serverID = ?", (guild.id,)
                        )
                        await sql.commit()
    ...
    async def enable_fireboard(self, interaction: discord.Interaction):
        ...
        if interaction.guild.id in [guild[0] for guild in self.fire_settings]:
            ...
            async with self.fireboard_pool.acquire() as sql:
                ...
                await sql.execute(
                    f"INSERT INTO fireSettings (serverID, reactAmount, emoji, channelID, ignoreBots) VALUES (?, ?, ?, ?, ?)",
                    (
        ...
        if interaction.guild.id in [guild[0] for guild in self.fire_settings]:
            ...
            try:
                ...
                async with self.fireboard_pool.acquire() as sql:
                    ...
                    await sql.execute(
                        f"UPDATE fireSettings SET emoji = ? WHERE serverID = ?",
                        (
                            str(reaction.emoji),
                            interaction.guild_id,
                        ),
    ...
    async def fireboard_channel(
        ...
        if interaction.guild.id in [guild[0] for guild in self.fire_settings]:
            ...
            async with self.fireboard_pool.acquire() as sql:
                ...
                await sql.execute(
                    f"UPDATE fireSettings SET channelID = ? WHERE serverID = ?",
                    (
                        channel.id,
                        interaction.guild_id,
                    ),
    ...
    async def fireboard_requirement(
        ...
        if interaction.guild.id in [guild[0] for guild in self.fire_settings]:
            ...
            async with self.fireboard_pool.acquire() as sql:
                ...
                await sql.execute(
                    f"UPDATE fireSettings SET reactAmount = ? WHERE serverID = ?",
                    (
                        amount,
                        interaction.guild_id,
                    ),
    ...
    async def fireboard_ignore_bots(
        ...
        if interaction.guild.id in [guild[0] for guild in self.fire_settings]:
            ...
            async with self.fireboard_pool.acquire() as sql:
                ...
                await sql.execute(
                    f"UPDATE fireSettings SET ignoreBots = ? WHERE serverID = ?",
                    (
                        value,
                        interaction.guild_id,
                    ),
        ...
        if interaction.guild_id in [guild[0] for guild in self.fire_settings]:
            async with self.fireboard_pool.acquire() as sql:
                if channel.id in [
                    ...
                    await sql.execute(
                        f"INSERT INTO fireChannelBlacklist (serverID, channelID) VALUES (?, ?)",
                        (
                            interaction.guild_id,
                            channel.id,
                        ),
        ...
        if interaction.guild_id in [guild[0] for guild in self.fire_settings]:
            async with self.fireboard_pool.acquire() as sql:
                if role.id in [roleEntry[1] for roleEntry in self.fire_role_blacklist]:
                    ...
                    await sql.execute(
                        f"INSERT INTO fireRoleBlacklist (serverID, roleID) VALUES (?, ?)",
                        (
                            interaction.guild_id,
                            role.id,
                        ),
